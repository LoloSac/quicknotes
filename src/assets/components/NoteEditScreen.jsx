...
export default function NoteEditScreen({ onAddNote, isEditing, onEdit, editingID, onCloseEdit, getNoteByID, onDelete}) {
    ...
    return (
        <div className= "fixed w-full h-full bg-black/50 flex flex-col justify-center items-center top-0 b-0" onMouseDown={onCloseEdit}>
            {/*mainWindow*/}
            <div className=" md:w-xl sm:w-md w-sm bg-surface p-8 flex flex-col gap-6 pb-6 relative rounded justify-evenly *:focus:outline-none *:focus:ring-2 *:focus:ring-primary/50" onMouseDown={(e) => e.stopPropagation()}>
                ...
                {/*BUTTONS*/}
                <div className="*:rounded *:h-10 *:w-20 *:text-base *:font-medium flex flex-row gap-2 sm:justify-end justify-around">
                    {/* Delete button */}
